...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse> handleUserNotFoundExceptions(RuntimeException ex) {
        return ResponseEntity.status(ErrorCode.USER_NOT_FOUND.getHttpStatusCode()).body(
                ApiResponse.builder()
    ...
    public ResponseEntity<ApiResponse> handleInvalidUUIDExceptions(RuntimeException ex) {
        return ResponseEntity
                .status(ErrorCode.INVALID_UUID_FORMAT.getHttpStatusCode())
                .body(ApiResponse.builder()
    ...
    public ResponseEntity<ApiResponse> handleUsernameExistsException(UsernameExistsException ex) {
        return ResponseEntity
                .status(ErrorCode.USERNAME_ALREADY_EXISTS.getHttpStatusCode())
                .body(ApiResponse.builder()
    ...
    public ResponseEntity<ApiResponse> handleAuthenticationFailedException(AuthenticationFailedException ex) {
        return ResponseEntity
                .status(ErrorCode.AUTHENTICATION_FAILED.getHttpStatusCode())
                .body(ApiResponse.builder()
    ...
    public ResponseEntity<ApiResponse> handleValidationException(MethodArgumentNotValidException exception) {
        ...
        List<Map<String, Object>> errors = exception.getBindingResult().getAllErrors().stream()
                .map(error -> {
                    ...
                    String message = error.getDefaultMessage(); // Get the detailed message set in the validator
